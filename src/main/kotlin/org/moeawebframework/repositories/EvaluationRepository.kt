...
interface EvaluationRepository : R2dbcRepository<Evaluation, Long> {
    ...
    @Query("SELECT `id`, `name`, `nfe`, `seeds`, `algorithm_id`, `problem_id`, `reference_set_id`, `status`, `results` FROM `evaluations` WHERE `user_id` = $1")
